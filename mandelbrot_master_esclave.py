...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        # rows_per_process = height // (size - 1)
        rows_per_process = 1
    
        deb = time()
        for i in range(1, size):
            start_row = (i - 1) * rows_per_process
            end_row = start_row + rows_per_process if i < size - 1 else height
            comm.send((start_row, end_row), dest=i)

        for i in range(1, size):
            start_row, end_row = comm.recv(source=i)
            convergence[start_row:end_row, :] = comm.recv(source=i)
        ...
        fin = time()
        print(f"Temps de reconstituittion d'image: {fin-deb}")
        image.save("output_master_esclave.png")
